...
public class GoodsController {
    ...
    public HttpResponse<List<GoodsVO>> getGoodsByKeyword(String keyword, int lastIndex, int size, long timestamp, HttpSession session) {
        LoginUtil.checkLogin(session);
        return new HttpResponse<>(goodsService.getGoodsByKeyword(keyword, lastIndex, size, timestamp));
    }

    @GetMapping("/getGoodsByCategory")
    public HttpResponse<List<GoodsVO>> getGoodsByCategory(String categoryID, int lastIndex, int size, long timestamp, HttpSession session) {
        LoginUtil.checkLogin(session);
        return new HttpResponse<>(goodsService.getGoodsByCategory(categoryID, lastIndex, size, timestamp));
    }
